...
# This gets the distance based on coordinates
class DirectionsTool(BaseTool):
    ...
    def _run(self, origin, destination):
        try:
            ...
            directions_result = gmaps.directions(origin, destination, mode="driving") ## by default we take distance by driving
...
directions_tool = DirectionsTool(description="Use to find travel distances and directions between two locations.")

google_maps_geocoding = GeocodingTool(description="Use to convert addresses into geographic coordinates.")
...
# Vector DB helps us to identify words that are closer to each other 
#in the vector embedding space. When we send it a list of strings, it creates this mathematical model of words- vector space
#FAISS - facebook AI similarity search. WE send the list of strings (alt & adresses)
#and initialize them in the VectorDB (in-memory)
def setup_tools(db, llm):
    ...
    # print(addresses)
    # print(alt_names)
    # logging.debug(f"Alt Names: {alt_names}")
    # logging.debug(f"Addresses: {addresses}")
    combined_texts = preprocess_texts(alt_names + addresses)
